...
text(M, y, 'Install guide for friends. Five minutes, one file, then it updates itself.', 'Body', 10.5, INK); y -= 0.42 * inch
steps = [
    ('Install Prism Launcher', 'Free, from prismlauncher.org/download. Open it and click through the first-run wizard.'),
    ('Sign in', 'Accounts (top right) > Add Microsoft. This is your normal Minecraft account.'),
    ('Import the pack', 'Add Instance > Import > choose LittleKettleValley.zip (do not unzip it) > OK.'),
    ('Set memory', 'Select the instance > Edit > Settings > Memory. 3072 MB on an 8 GB Mac, 3584 on 16 GB, 4096 on 32 GB.'),
    ('Launch', 'First launch downloads about 125 mods and takes a few minutes. After that it is fast, and every launch pulls updates for you.'),
]
for i, (h1, body) in enumerate(steps, 1):
    c.setFillColor(COPPER); c.circle(M + 0.16 * inch, y - 0.02 * inch, 0.16 * inch, fill=1, stroke=0)
    text(M + 0.1 * inch, y - 0.07 * inch, str(i), 'Rounded', 12, CREAM)
    text(M + 0.45 * inch, y, h1, 'Rounded', 13, INK); y -= 0.24 * inch
    y = wrap(M + 0.45 * inch, y, body, W - 2 * M - 0.45 * inch, 'Body', 10.5, INK, 15) - 0.14 * inch
BOX_H = 1.05 * inch; by = y - 0.05 * inch - BOX_H
...
ty = wrap(M + 0.2 * inch, ty, 'Multiplayer > Add Server > paste the address Josh sends you. Your name has to be on the whitelist, so tell Josh your exact Minecraft username.', W - 2 * M - 0.4 * inch, 'Body', 10, INK, 14)
footer(1); c.showPage()
# ---------- page 2: what it is + if it won't start
page_bg(); y = H - M - 0.1 * inch
if mark.exists():
    im = ImageReader(str(mark)); c.drawImage(im, W - M - 0.9 * inch, y - 0.9 * inch, 0.9 * inch, 0.9 * inch, mask='auto')
text(M, y - 0.25 * inch, 'What you are walking into', 'Rounded', 16, COPPER); y -= 0.55 * inch
y = wrap(M, y, 'You inherit a cold house in a quiet valley. A letter, a copper kettle, and forty dark lamp posts. The quest book (press J) always tells you the one next thing to do. Follow it and the town wakes up one neighbour at a time. Forty lamps. Fifteen people. One winter that nobody leaves.', W - 2 * M - 1.0 * inch, 'Body', 10.5, INK, 16) - 0.2 * inch
...
for t in fixes:
    ...
    y = wrap(M + 0.22 * inch, y, t, W - 2 * M - 0.22 * inch, 'Body', 10, INK, 14) - 0.06 * inch
footer(2); c.save(); print('wrote', out)
