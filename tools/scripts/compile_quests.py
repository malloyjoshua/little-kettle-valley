...
GROUP_ORDER = ['The Valley', 'Home and Farm', 'Tech', 'The Valley Beyond', 'Side Quests', 'Story']
...
# The KubeJS id map is written INTO THE PACK WE JUST COMPILED FOR, derived from the
# output dir (<pack>/config/ftbquests/quests -> <pack>/kubejs/server_scripts), never a
# hardcoded 'pack/'. The old hardcoded path meant compiling ANY other book from the repo
# root silently overwrote the story pack's map with the other book's keys and broke every
# global.valleyQuestIds lookup in valley_*.js.
# It is only written when that pack actually reads the map: the map exists to serve
# global.valleyQuestIds lookups, so a pack with no such lookup (Kettle Tech) gets no file.
scripts_dir = out.parent.parent.parent / 'kubejs' / 'server_scripts'
idmap = scripts_dir / '_quest_ids.js'
wants_idmap = scripts_dir.is_dir() and any(
    'valleyQuestIds' in p.read_text(errors='ignore') or 'valleyChapterIds' in p.read_text(errors='ignore')
    for p in scripts_dir.glob('*.js') if p.name != '_quest_ids.js')
if wants_idmap:
    ...
    idmap.unlink(); print(f"removed unused {idmap}")
