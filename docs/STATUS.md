...
### Pacing and playability pass (2026-09-06, 09:40-10:45)
Three audits walked every chapter with exact fixes (`scratch/audit_newcomer.md`, `audit_veteran.md`, `audit_coop.md`, all tracked); appliers landed 136 of them, then compile, feasibility, lint and hooks were re-run clean.
- **Newcomer (wife, MacBook Air):** Start Here + Act I now about 2 h 35 m with a felt win every 8-10 minutes in the first hour. Fixed: no crafting table for the soup's bowl (now given with the pot), Andesite Casing's right-click recipe never stated, nine seeds for a 27-tile patch, "optional" quests that gated act finales, F3 and Home keys that do not exist on a Mac laptop, controls never taught, seasons' 27-day length never stated, the animal chapter's root needing a slimeball.
- **Veteran (Josh):** first machine ~1 h on the story road (35 min beelining Create); first Thermal machine ~2 h 10 (45 min beeline); first ME terminal ~4 h 10 (2 h 30 beeline); reactor lit ~7 h 30; the story spine ~10 h 45; all 22 chapters 26-30 h. Fixed: the turbine quest never said how RPM is controlled (blocker), no Energy Acceptor in the AE2 story, no itemduct anywhere, 64 uranium and other grind counts, the Act III finale paying nothing, the reactor chapter not saying it hangs off Josie's notes, a Nether fortress mid-Create chain.
- **Co-op:** every scene, finale, resident line and sound now fires once per party instead of once per player (73 command rewards teamed); scrip pays once; one Surveyor's Stake per party; two players joining in the same second no longer end up on two parties; the book now says what to do when one of you plays ahead.
- Still yours: the actual first hour by a human. And `rm_lanes` and q71 are the two longest pages in the book on purpose; trim them if they read heavy.
