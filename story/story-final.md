...
**The Surveyor's Stake.** "Place it on flat-ish ground" is a builder's judgment call, and it is the most consequential placement in the pack — 99 quests offset from it. So the judgment is removed. *(Shipped form: a block that tints the terrain needs a client-side renderer, which this pack does not have, so the signal is server-side and the check is `stakeGuide` in `valley_checks.js`. While the world has no anchor and the Stake is in hand, standing 30–96 blocks north of Home on a level 5×5 pad with headroom throws a ring of green `happy_villager` particles around her feet and Josie says so once. If the ground up there never offers a pad, Josie says after ~30 seconds that levelling a 5×5 herself is allowed. The stake still places anywhere — the sparks are the answer, never a lock.)*
...
| Q7 | Where the Square Goes | B | Bram's old **Surveyor's Stake**. Walk north up the road from your gate until green sparks come up around your feet, then right-click in them. This is where the town will be. | Anchor recorded. A stone path is laid from your door to the anchor and **the first two lamp posts go up (2/40)** | Q6 |
...
| Q26 | Dredging the Shallows | C | Sit in Nella's boat and pull the dredge net eight times (16 Lake Sand a pull). You never get in the water. | 128 Lake Sand back, a boat, stage `dredged` | Q22 |
